...
| Ops      | Optional **Sentry** error reporting (`sentry/sentry-laravel`, `SENTRY_LARAVEL_DSN`, `config/sentry.php`); transactional email via Laravel mailer — **SMTP** (`MAIL_*`) or **Brevo API** (`MAIL_MAILER=brevo`, `BREVO_API_KEY`, same pattern as Issue Forge; see README); **GitHub Actions CI** (PHPUnit + Vite build + **`composer audit`** + **Laravel Pint** on `dev` / `master`); **Dependabot** for Composer, npm, and Actions |
